...
/* ------------------
	HEADER/FOOTER - commenting out because I'm already handling header and footer stuff with my own code!
------------------ */
...
if (tagsList) {
	...
	<li id="${formatTagHash(tag)}">
		<details>
			<summary>${tag}</summary>
			<ul class="post-list" reversed>${[...postIndices].map(i => formatPostLink(posts[i])).join(``)}</ul>
		</details>
	</li>
	`).join(``);
...
/* Attempting to get my own in-post tags list to work */

if (document.getElementById(`postTags`)) {
	const i = posts.findIndex(post => post.filename === path.at(-1).split(`.html`)[0]);
	if (posts[i].tags) document.getElementById(`postTags`).innerHTML = posts[i].tags.map(tag => `${tag}`).join(`, `);
}